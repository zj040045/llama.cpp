    ...
    auto close_ofstream = [&]() {
        ...
        if (fout.is_open()) {
            if (params->keep_split) {
                // Set split info
                gguf_set_val_u16(ctx_outs[cur_split], ml.llm_kv(LLM_KV_SPLIT_NO).c_str(), cur_split);
                gguf_set_val_u16(ctx_outs[cur_split], ml.llm_kv(LLM_KV_SPLIT_COUNT).c_str(), n_split);
                gguf_set_val_i32(ctx_outs[cur_split], ml.llm_kv(LLM_KV_SPLIT_TENSORS_COUNT).c_str(), ml.n_tensors);
            }
            fout.seekp(0);
